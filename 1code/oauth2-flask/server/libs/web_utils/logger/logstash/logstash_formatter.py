...
class LogstashFormatterV1(LogstashFormatter):
    ...
    def format(self, record):
        ...
        # 只需要关键几个
        logrr = {
            '@timestamp':logr.get('@timestamp'),                # 时间
            '@version':logr.get('@version'),
            
            'user_openid':logr.get('user_openid'),              # 人物1
            'user_nickname':logr.get('user_nickname'),          # 人物2
            'http_authorization':logr.get('http_authorization'),# 人物3

            'api_name':logr.get('api_name'),                    # 起因1
            'api_url':logr.get('api_url'),                      # 起因2

            'errcode':logr.get('errcode'),                      # 结果1
            'http_code':logr.get('http_code'),                  # 结果2
            'message':logr.get('message'),                      # 结果3

            'error_message':logr.get('error_message'),          # 结果3
            'error_traceback':logr.get('error_traceback'),      # 结果3，地点

            'arguments':logr.get('arguments'),      # 参数
        }


        return json.dumps(logrr, default=self.json_default, cls=self.json_cls)
